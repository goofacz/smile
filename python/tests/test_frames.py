...
class TestFrames(unittest.TestCase):
    def test_load_csv(self):
        ...
        self.assertTupleEqual((2, 15), nodes.shape)

        # Check field access
        np.testing.assert_equal((hash('RX'), hash('TX')), nodes[:, Frames.DIRECTION])

        self.assertEqual(12345678, nodes[0, Frames.NODE_MAC_ADDRESS])
        self.assertEqual(98765432, nodes[1, Frames.NODE_MAC_ADDRESS])
